...
public class Remote {
    ...
    static CookieManager sCookieManager = new CookieManager();
    ...
    public static String postData(String webUrl, Map params, String method) throws IOException{
        ...
        // Cookie 매니저에 저장 : 앱을 종료 시키면 유지 안됨
        Map<String, List<String>> headers = connection.getHeaderFields();
        List<String> cookies = headers.get("Set-Cookie");
        if (cookies != null && cookies.size() > 0){
            for (String cookie : cookies){
                Log.i(TAG, "HttpCookie:"+HttpCookie.parse(cookie).get(0));
                sCookieManager.getCookieStore().add(URI.create(COOKIE_URL), HttpCookie.parse(cookie).get(0));
            }
        }
